...
for epoch in tqdm(range(1, n_epochs+1)):
    ...
    test_accuracy.append(100 * right_sample/total_sample)
    ######################    
    # 验证集的模型#
    ######################
    ...
    for data, target in valid_loader:
        ...
        for i in correct_tensor:
            if i:
                right_sample += 1
    print("Accuracy:",100*right_sample/total_sample,"%")
    train_accuracy.append(100 * right_sample/total_sample)
    ...
    if valid_loss <= valid_loss_min:
        ...
        counter += 1
    prop["Loss/train"].append(train_loss)
    prop["Accuracy/train"].append(train_accuracy)
    prop["Loss/test"].append(valid_loss)
    prop["Accuracy/test"].append(test_accuracy)
    print(prop)
...
# train/accuracy
plt.plot(prop["Accuracy/train"][1])
plt.grid() # 显示网格
...
plt.ylabel('Loss') # 显示y轴标签
plt.xlabel('Epoch')
plt.title('Loss function of test')
ax=plt.gca()
ax.xaxis.set_major_locator(x_major_locator)
